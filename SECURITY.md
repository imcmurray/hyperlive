...
| **`setStageSource` (overlay mode)** | The one door that loads **remote** content (a YouTube video / video URL as the stage background). It is **operator-only**: allowlisted on the loopback `/mutate`, but **never emitted by the director**, so a viewer can't set the source. Inputs are strictly whitelisted — a YouTube id must match `[A-Za-z0-9_-]{11}`, other URLs must be `http(s)` — and elements are built with DOM APIs (never `innerHTML`), so neither markup nor a `javascript:`/`data:` source can be smuggled in. A YouTube source is resolved server-side by `yt-dlp` to a direct media URL the scene plays in a `<video>` (so its audio is captured). |

**A note on `--disable-web-security`.** In source-audio mode (`AUDIO_MODE=source`), the capture browser launches with `--disable-web-security` because the resolved HLS/googlevideo media URLs send no CORS headers and hls.js can't otherwise fetch them. This is safe **on this specific browser** because it is a capture-only rig: the top frame runs only our own trusted JS (the scene, gsap, hls.js, the YouTube IFrame API), and the only untrusted content — viewer-authored cards — is isolated in `sandbox=""` iframes with `default-src 'none'` CSP, which this flag does not affect. It is gated to source-audio mode, so it is off entirely unless overlay-with-sound is in use. Do not reuse this browser profile for general web browsing.
